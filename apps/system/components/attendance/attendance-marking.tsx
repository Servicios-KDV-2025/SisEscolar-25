...
export default function AttendanceMarking({
  ...
  const markAttendance = (studentClassId: string, present: boolean) => {
    const existingRecord = getTodayAttendance(studentClassId)
    const now = Date.now()
    ...
    if (existingRecord) {
      ...
      setAttendanceRecords(
        attendanceRecords.map((record) =>
          record.studentClassId === studentClassId && new Date(record.date).setHours(0, 0, 0, 0) === today
            ? {
                ...record,
                present,
                justified: present ? undefined : false, // Reset justified status for new absences
                comments: comments[studentClassId] || record.comments,
      ...
      const newRecord: AttendanceRecord = {
        ...
        date: now,
        present,
        justified: present ? undefined : false,
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          Matricula: {studentClass.student.rollNumber} | Clase: {studentClass.className}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex space-x-2">
                        <Button
                          ...
                          onClick={() => markAttendance(studentClass.id, true)}
                        ...
                        <Button
                          ...
                          onClick={() => markAttendance(studentClass.id, false)}
                          className="flex items-center space-x-1"
                        >
                          <X className="h-4 w-4" />
                          <span>Ausente</span>
