...
export default function ClassroomManagement() {
  ...
  const [sortBy, setSortBy] = useState<
    "name" | "location" | "capacity" | "createdAt"
  >("location");
  ...
  const handleSubmit = async (values: Record<string, unknown>) => {
    if (!currentSchool?.school._id) {
      toast.error(
        "No se encontró la escuela actual. Refresca e intenta de nuevo."
      );
      return;
    }
    ...
    if (
      existingClassroom(validData.name, validData.location) &&
      (operation !== "edit" ||
        (data &&
          (data.name !== validData.name ||
            data.location !== validData.location)))
    ) {
  ...
  const handleSort = (
    column: "name" | "location" | "capacity" | "createdAt"
  ) => {
    if (sortBy === column) {
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg text-muted-foreground">
                        Administra las aulas de {currentSchool?.school?.name}{" "}
                        con sus atributos de capacidad, ubicación y estado.
                      </p>
                    </div>
                  </div>
                </div>
