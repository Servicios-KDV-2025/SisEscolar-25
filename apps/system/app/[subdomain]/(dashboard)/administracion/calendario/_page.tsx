  ...
  const datosCalendario = useMemo(() => {
    if (!eventos)
      return {
        ...
        eventosDelDia: [] as CalendarType[], // <-- Tipado aquí
        eventosFiltrados: [],
      };

    const fechasConEventos = new Map<string, string>();
    const contadorEventos: Record<string, number> = {};
    const eventosDelDia: CalendarType[] = []; 
  
    const fechaSeleccionada = selectedDate ? selectedDate : new Date();

    eventos.forEach((evento: CalendarType) => {
        const tipoEvento = getTipoEventoById(evento.eventTypeId);
        const tipoClave = tipoEvento?.key || "default";

        const diasDelEvento = eachDayOfInterval({
        ...
        diasDelEvento.forEach((dia) => {
          ...
          fechasConEventos.set(fecha, tipoClave);
        });

        contadorEventos[tipoClave] = (contadorEventos[tipoClave] || 0) + 1;
  
        const estaEnElRango = diasDelEvento.some((diaDelEvento) =>
          isSameDay(fechaSeleccionada, diaDelEvento)
        );
  
        if (estaEnElRango) {
          eventosDelDia.push(evento); // <-- Ahora 'eventosDelDia' sí existe
        }
      });
  
      return { fechasConEventos, contadorEventos, eventosDelDia, eventos }; // <-- Y aquí también
                  ...
                  <CardContent>
                    {datosCalendario.eventosDelDia.length > 0 ? (
                      <div className="space-y-3 max-h-84 overflow-y-auto">
                        {datosCalendario.eventosDelDia.map((evento: CalendarType,) => {
                ...
                <CardContent>
                  ...
                  datosCalendario.eventos?.length > 0 ? (
                    <div className="space-y-3 max-h-84 overflow-y-auto">
                      {datosCalendario.eventos
                        ?.filter(
                          (evento) => new Date(evento.startDate) >= new Date()
                        )
                        .sort(
                          (a, b) =>
