...
export const PaymentStatusBlock: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    fetch("/api/payment-session", {
      ...
      .then((res) => {
        if (!res.ok) throw new Error("Error fetching payment session");
        return res.json();
      })
      .then((data) => {
        setPaymentInfo(data);
  ...
  image?.asset ? (urlForImage(image as any)?.auto?.("format")?.url() ?? null) : null;
const imageAlt = image?.alt || title || "Imagen";
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 opacity-5 ${isSuccess ? "bg-emerald-500" : "bg-rose-500"}`}
        style={{
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          {imageUrl ? (
            ...
            >
              <div className={`text-5xl ${isSuccess ? "text-emerald-600" : "text-rose-600"}`}>
                {isSuccess ? "✓" : "✕"}
              </div>
            </div>
          )}
            ...
            <div className="bg-white/90 backdrop-blur-sm border border-white/30 shadow-2xl rounded-3xl p-8">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center py-3 border-b border-gray-100">
                  ...
                  <span className="text-gray-800 font-semibold">{paymentInfo.email ?? "N/A"}</span>
                </div>
                ...
                <div className="flex justify-between items-center py-3">
                  ...
                  <span className="text-gray-800 font-mono text-sm bg-gray-50 px-3 py-2 rounded-lg">
                    {paymentInfo.subscriptionId ?? "N/A"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/90 backdrop-blur-sm border border-white/30 shadow-2xl rounded-3xl p-8">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center py-3 border-b border-gray-100">
                  ...
                  <span className="text-gray-800 font-semibold">
                    {paymentInfo.subscription_current_period_start
                      ? new Date(paymentInfo.subscription_current_period_start * 1000).toLocaleDateString("es-ES", {
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                        })
                ...
                <div className="flex justify-between items-center py-3 border-b border-gray-100">
                  ...
                  <span className="text-gray-800 font-semibold">
                    {paymentInfo.subscription_current_period_end
                      ? new Date(paymentInfo.subscription_current_period_end * 1000).toLocaleDateString("es-ES", {
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                        })
          ...
          <div className="bg-white/90 backdrop-blur-sm border border-white/30 shadow-2xl rounded-3xl p-8 mb-12">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6 max-w-2xl mx-auto">
                Si tienes alguna pregunta sobre el proceso de pago o necesitas asistencia, nuestro equipo de soporte está aquí para ayudarte.
              </p>
        ...
        {/* Footer */}
        <div className="text-center">
          <p className="text-gray-500 text-sm">Si tienes alguna pregunta, no dudes en contactarnos</p>
        </div>
      </div>
    </section>
  );
