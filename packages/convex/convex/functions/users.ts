...
export const deleteUser = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (studentsAsTutor.length > 0) {
            throw new Error("No se puede eliminar el usuario porque es tutor de estudiantes");
        }
        ...
        if (userSchools.length > 0) {
            throw new Error("No se puede eliminar el usuario porque tiene roles asignados en escuelas");
        }
...
export const getUserStats = query({
    ...
    handler: async (ctx) => {
        ...
        const stats = {
            ...
            withEmail: allUsers.filter(user => user.email && user.email.length > 0).length,
            withPhone: allUsers.filter(user => user.phone && user.phone.length > 0).length,
            withImage: allUsers.filter(user => user.imgUrl && user.imgUrl.length > 0).length,
        };
